...
void sserial_send_start(unsigned char portindex){HAL_GPIO_WritePin(GPIOC, GPIO_PIN_12, GPIO_PIN_SET);};
void sserial_send_end(unsigned char portindex){HAL_GPIO_WritePin(GPIOC, GPIO_PIN_12, GPIO_PIN_RESET);};
void var_delay_ms(int ms){HAL_Delay(ms);}
char sserial_process_internal();

void uart_send(unsigned char port, char ch)
{
		uint8_t data[1];
		data[0] = ch;
...
uint8_t uart_get(unsigned char port)
{
		return uart_byte;
}

char uart_received(unsigned char port){return 1;}
